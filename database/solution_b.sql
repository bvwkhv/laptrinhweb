...
--câu 3
SELECT order_id AS ma_don_hang, COUNT(product_id) AS so_san_pham 
FROM order_details 
GROUP BY order_id;
...
ORDER BY so_luong_don_hang DESC LIMIT 7;

--câu 6
SELECT u.user_id, u.user_name, o.order_id, p.product_name 
FROM users AS u 
JOIN orders AS o ON u.user_id = o.user_id 
JOIN order_details AS od ON o.order_id = od.order_id J
OIN products AS p ON od.product_id = p.product_id 
WHERE (p.product_name LIKE '%Samsung%' OR p.product_name LIKE '%Apple%') 
ORDER BY o.order_id DESC LIMIT 7;

--câu 7
SELECT u.user_id, u.user_name, o.order_id, SUM(p.product_price) AS tong_tien_don_hang 
FROM users AS u 
    JOIN orders AS o ON u.user_id = o.user_id 
    JOIN order_details AS od ON o.order_id = od.order_id 
    JOIN products AS p ON od.product_id = p.product_id 
...
--câu 9
SELECT user_id, user_name, order_id, total_amount, total_products
FROM (
    SELECT 
        u.user_id,
        u.user_name,
